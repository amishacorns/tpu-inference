...
- The MoE feed-forward's grouped matmuls fused into one kernel. Gate and
  ...
  every call.
- The gated delta network's convolution state and recurrent state passed
  in their stored layout, the recurrent state stored in bfloat16,
  widened on load and rounded on writeback. The stored-layout handling
  is always on, the flag governs only the bfloat16 state.
