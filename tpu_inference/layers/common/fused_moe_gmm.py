...
def moe_gmm_local(x: jax.Array,
    ...
    bits. On for every call that fits it, with no off switch.
    """
    ...
    if not fuse_gmm_pair:
        logger.info_once(
            "The fused MoE FFN kernel cannot take this layer (%s); running "
            "the two grouped matmuls instead.", fuse_reason)
    if fuse_gmm_pair:
