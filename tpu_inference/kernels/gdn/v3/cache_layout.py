...
def conv_state_layout(cache_shape, dtype) -> "Layout | None":
    ...
    derived from the element size. Returns None, after a warning once per
    message, for a cache this is not defined for; it is then allocated in
    the default layout rather than failing the boot. The messages name the
    first state of the mamba cache rather than calling it a convolution
    state, since a model whose first state is something else is exactly
