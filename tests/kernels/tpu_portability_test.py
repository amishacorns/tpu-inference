...
# The fused grouped-matmul kernel ships in its own change. On a tree that
# does not carry it yet, its checks skip by name rather than failing this
# file's import, and they run again as soon as the modules are present.
try:
    from tpu_inference.kernels.megablox.gmm_fused import (
        default_vmem_limit_bytes, gmm_fused, unsupported_reason)
    from tpu_inference.kernels.megablox.gmm_v2_fused_support import (
        sublane_tiling, validate_inputs)
    _HAS_FUSED_GMM = True
except ImportError:
    _HAS_FUSED_GMM = False

needs_fused_gmm = pytest.mark.skipif(
    not _HAS_FUSED_GMM,
    reason="this tree does not carry the fused grouped-matmul kernel")
