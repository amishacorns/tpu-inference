...
def gdn_local_operands(max_reqs,
    ...
    Shared by every comparison below, so all of them see the same inputs.
    `context_lens` gives each sequence the tokens already behind it, which
    is what makes the kernel read the slot it is about to write.
    `stale_states` fills the slots with values instead of zeros, which
    only matters alongside a context length.
    """
    kq_head_dim = 128
    v_head_dim = 128
    n_kq = 2
...
class GDNAttentionTest(parameterized.TestCase):
    ...
    def test_conv_cache_native_matches_converted_operand(
        ...
        The cache is bfloat16, which is what the serving path allocates.
        The converted form widens the whole cache outside the kernel and
        narrows it again on the way out; the cache-native form does both
        inside, on the slot being read. It is the same rounding either
        way, so this is an equality and not a tolerance. The
        window-boundary case runs sequences of two, three and four tokens
        against a convolution window of kernel_size - 1 = 3 rows, which
        is where how much of the stored window survives changes.
        """
...
class ReadOffsetGateTest(BoundaryTestCase):
    ...
    @parameterized.named_parameters(
        ("with_spec_decoding", 4),
        ("with_one_speculative_token", 1),
    )
    def test_read_offset_accepted(self, num_spec_tokens):
        """Speculative decoding passes an offset, at any window size.
        ...
        self.assert_accepted(boundary_operands(),
                             read_offsets=jax.ShapeDtypeStruct((NUM_SEQS, ),
                                                               jnp.int32),
                             num_spec_tokens=num_spec_tokens)
...
class ConvStateLayoutTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(("rank_four", (17, 3, 512, 1)), )
    ...
    def test_element_width_with_no_defined_layout_declines(self):
        with self.assertLogs(CACHE_LAYOUT_LOGGER, level="WARNING") as logs:
            self.assertIsNone(
                cache_layout.conv_state_layout(self.CACHE_SHAPE, jnp.float64))
