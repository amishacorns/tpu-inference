...
class UnsupportedReasonTest(jtu.JaxTestCase):
    ...
    def test_a_row_count_that_is_not_whole_sublanes_fails_loudly(
            self, num_rows):
        """Above the tile height such a row count has no layout: the gate
        does not name it, so the build must fail rather than mis-shape."""
        shapes = ffn_shapes(num_rows=num_rows)
        with pinned_tpu():
            self.assertIsNone(reason_for(shapes))
            with self.assertRaisesRegex(
                    ValueError, "evenly divide|multiple of the sublane"):
                trace_gmm_fused(shapes)
